...
RSpec.describe EmailChange, type: :model do
  describe '#valid?' do
    subject { email_change.valid? }
    let(:email_change) { build(:email_change) }

    it { is_expected.to be_truthy }

    context 'when email_changeable is blank' do
      let(:email_change) { build(:email_change, email_changeable: nil) }

      it { is_expected.to be_falsey }
    end

    context 'when change_digest is blank' do
      let(:email_change) { build(:email_change, change_digest: '') }

      it { is_expected.to be_falsey }
    end

    context 'when change_digest is not unique' do
      let(:existing_email_change) { create(:email_change) }
      let(:email_change) { build(:email_change, change_digest: existing_email_change.change_digest) }

      it { is_expected.to be_falsey }
    end

    context 'when email is blank' do
      let(:email_change) { build(:email_change, email: '') }

      it { is_expected.to be_falsey }
    end
  end

  describe '#email_changeable' do
    subject { email_change.email_changeable }
  ...
  describe '.detected_by' do
    ...
    let(:token) { 'token' }
    let!(:email_change) { create(:email_change, change_digest: described_class.digest('token')) }
  ...
  describe '#match?' do
    ...
    context 'when token is incorrect' do
      let(:email_change) { create(:email_change, change_digest: described_class.digest('otherToken')) }
